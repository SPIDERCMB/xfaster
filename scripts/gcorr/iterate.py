...
for tag in tags:
    ...
    try:
        gcorr_corr = xf.load_and_parse(
            os.path.join(rundirf, "gcorr_{}.npz".format(tag))
        )
        print(
    ...
    fig_tot.savefig(
        os.path.join(
            rundir,
            "plots",
            "gcorr_tot_{}_{}.png".format(tag, os.get_environ("GCORR_ITER")),
        )
    )
    fig_corr.savefig(
        os.path.join(
            rundir,
            "plots",
            "gcorr_corr_{}_{}.png".format(tag, os.get_environ("GCORR_ITER")),
        )
    )
...
os.system(
    "python run_xfaster.py --gcorr-config {g} --omp 1 --check-point transfer -o {o} > /dev/null".format(
