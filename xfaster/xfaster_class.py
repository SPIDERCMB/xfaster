...
class XFaster(object):
    ...
    def get_masked_sims(
        ...
        if do_fg:
            opts.update(foreground_type=foreground_type, foreground_subset=foreground_subset)
            file_attrs += [
                "foreground_type",
                "foreground_subset",
                "foreground_root",
        ...
        if hasattr(self, "signal_type") and self.signal_type == signal_type:
            if not hasattr(self, "foreground_type") or self.foreground_type == foreground_type:
    ...
    def get_signal_shape(
        ...
        if "fg" in comps:
            ...
            if filename_fg is None:
                ...
                if not os.path.exists(filename):
                    raise OSError("Missing foreground model file {}".format(filename))
